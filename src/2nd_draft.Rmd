...
Fligner-Killeen Test was used to test the homogeneity of variances. The test does not rely on the assumption of normality and thus it is more robust to non-normal data. We applied the test to each categorical variable to test if there is a equal variance aross all sub-groups in each variable. If p-value is less than 5\% for one categorical variable, then at 5\% significane level, there is a significant difference of variance aross groups for that variable. 
...
Also, another advantage of these models is that MSPE is more stable as the variance of MSPE is reduced. For more detials about the two models, please refer to **Reference**.
...
Here categorical variables are taken into account, boxplots of mill rate across municipalities and tax classes are plotted to display the distributions in Figure 3 and Figure 4.
...
```{r,echo=F, warning=F}
hist(assessment_transform$rate, main = "Figure 5: Histogram of rate", xlab="Mill Rate")
qqnorm(assessment_transform$rate, main = "Figure 6: Normal Q-Q Plot of rate", ylab = "Mill Rate")
...
attach(assessment_transform)
fligner.test(rate~Year) # p-value = 0.004907
fligner.test(rate~TaxClassCode) # p-value < 2.2e-16
fligner.test(rate~AddressAssessorMunicipalityDesc) # p-value = 0.1299
```
...
Hence, assumptions of linear regression are not met in this study. This means the test results, for example, t-statisitcs and p-values, are biased and thus cannot be trusted. Since this study only focuses on prediction accuracy, the failure of linear regression assumptions might not matter much. 
