...
(i) linear model
```{r}
# based on EDA, we chose to fit model using municipality, tax class code, avg assess value
linear_full<-lm(rate~factor(AddressAssessorMunicipalityDesc)+factor(TaxClassCode)+AvgTotalAssessmentValue,data=assessment_transform)
summary(linear_full)
...
```{r}
dummy_year<-dummy(assessment_transform$Year)
...
# build x matrix
x<-cbind(dummy_municipal,dummy_year,dummy_taxclass,assessment_transform$AvgTotalAssessmentValue)
...
colnames(x)[30]<-c("AvgTotalAssessmentValue")
xm<-x[,1:(ncol(x)-1)]
x.data.frame<-assessment_transform
xm.data.frame<-assessment_transform[,-8]
...
for (i in 1:N) {
  ...
  for (j in 1:k) {
    ...
    tmp.full <- lm(rate ~ factor(AddressAssessorMunicipalityDesc)+factor(Year)+factor(TaxClassCode)+assessTotal+landTotal+improvementTotal+propertyCount+AvgTotalAssessmentValue, 
                   data = x.data.frame[ii != j, ])
...
boxplot(mspe.la, 
        ...
        col = c("steelblue","gray80","pink", "tomato", "springgreen"), 
        cex.axis = 1, 
        cex.lab = 1, 
        cex.main = 2)
