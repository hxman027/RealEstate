...
linear_full<-lm(rate~factor(AddressAssessorMunicipalityDesc)+factor(Year)+factor(TaxClassCode)+assessTotal+landTotal+improvementTotal+propertyCount+predTaxIncome,data=assessment_aggregate)
summary(linear_full)
...
```
Multiple R-squared:  0.8874,	Adjusted R-squared:  0.8708 
MSPE = 1.984109

Next: variable selection
with only significant variables
```{r}
reduced<-lm(rate~factor(Year)+factor(TaxClassCode)+factor(AddressAssessorMunicipalityDesc), data=assessment_aggregate)
summary(reduced)

reduced_fit<-augment(reduced)
sqrt(sum((reduced_fit$.resid)^2)/nrow(assessment_aggregate))
```
Multiple R-squared:  0.8826,	Adjusted R-squared:  0.8679 
MSPE = 2.02679
With fewer variables, adjusted R-squared declines.


By comparing the two models, the fit of full model is the better of the two.
...
# build x matrix
x<-cbind(dummy_municipal,dummy_year,dummy_taxclass,assessment_aggregate$assessTotal,assessment_aggregate$landTotal,assessment_aggregate$improvementTotal,assessment_aggregate$propertyCount,assessment_aggregate$predTaxIncome,assessment_aggregate$tax)
...
elastic_net = train(
  rate~factor(AddressAssessorMunicipalityDesc)+factor(Year)+factor(TaxClassCode)+assessTotal+landTotal+improvementTotal+propertyCount+predTaxIncome, data = assessment_aggregate,
  method = "glmnet",
  trControl = cv_10
)
elastic_net
...
The final values used for the model were alpha = 0.1 and lambda = 0.007102922.
 alpha  lambda       RMSE      Rsquared   MAE     
  0.10   0.007102922  2.238720  0.8623328  1.711568

Among these all the models, lasso seems to have the best goodness of fit.
...
newx<-test[,-c(8)]
y<-test[,c(8)]
linear_1<-lm(rate~factor(AddressAssessorMunicipalityDesc)+factor(Year)+factor(TaxClassCode)+assessTotal+landTotal+improvementTotal+propertyCount+predTaxIncome,data=train)
...
```{r}
# reduced variable
linear_2<-lm(rate~factor(Year)+factor(TaxClassCode)+factor(AddressAssessorMunicipalityDesc),data=train)
...
cv_10 = trainControl(method = "cv", number = 10)
elastic_net = train(
 rate~factor(AddressAssessorMunicipalityDesc)+factor(Year)+factor(TaxClassCode)+assessTotal+landTotal+improvementTotal+propertyCount+predTaxIncome, 
...
# The final values used for the model were alpha = 1 and lambda = 0.006549203.

elastic_reg = glmnet(x_train, y_train, nlambda = 25, alpha = 1, family = 'gaussian', lambda =  0.006549203)
