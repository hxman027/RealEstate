...
linear_full<-lm(rate~.,data=assessment_aggregate)
summary(linear_full)


library(broom)
linear_full_fit<-augment(linear_full)
sqrt(sum((linear_full_fit$.resid)^2)/nrow(assessment_aggregate)) #  2.772333


```
Multiple R-squared:  0.7803,	Adjusted R-squared:  0.7503 
MSPE = 2.772333

Next: variable selection
with only significant variables
```{r}
reduced<-lm(rate~Year+TaxClassCode+AddressAssessorMunicipalityDesc, data=assessment_aggregate)
summary(reduced)

reduced_fit<-augment(reduced)
sqrt(sum((reduced_fit$.resid)^2)/nrow(assessment_aggregate)) #  2.861619


```
Multiple R-squared:  0.7659,	Adjusted R-squared:  0.7408 
MSPE = 2.861619
With fewer variables, adjusted R-squared declines.


By comparing the two models, the fit of full model is better.
...
elastic_net = train(
  rate~., data = assessment_aggregate,
  method = "glmnet",
  trControl = cv_10
)
elastic_net
```
The final values used for the model were alpha = 0.1 and lambda = 0.09334029.
MSPE = 3.066083  
R squared = 0.7459584
...
```{r}
# reduced variable
linear_2<-lm(rate~Year+TaxClassCode+AddressAssessorMunicipalityDesc,data=train)
...
cv_10 = trainControl(method = "cv", number = 10)
elastic_net = train(
  rate~., data = assessment_aggregate[train_ind,],
  method = "glmnet",
  trControl = cv_10
)
elastic_net
# RMSE was used to select the optimal model using the smallest value.
# The final values used for the model were alpha = 0.55 and lambda = 0.009140377.

ridge_reg = glmnet(x_train, y_train, nlambda = 25, alpha = 0.55, family = 'gaussian', lambda = 0.009140377)
predictions_test <- predict(ridge_reg, newx = x_test)
eval_results(y_test, predictions_test)
```
MSPE = 2.566096	
