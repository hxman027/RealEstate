...
sqrt(sum((linear1_fit$.resid)^2)/nrow(pct_final)) # 5.614843
linear2_fit<-augment(linear2)
sqrt(sum((linear2_fit$.resid)^2)/nrow(pct_final)) # 5.614843

```
linear 1: Multiple R-squared:  0.4796,	Adjusted R-squared:  0.155 
linear 2: Multiple R-squared:  0.4796,	Adjusted R-squared:  0.155

we re-looked at the data and found percentage changes of tax in "Langley - City" and "Langley - Township" are too large (~400). We ignored the two municipalities and refit the linear model and observed that r^2 improved to 0.4882 (Multiple R-squared:  0.4882,	Adjusted R-squared:  0.1605).
Still not too much difference bewteen the model with and without variable year.
...
sqrt(sum((fit_linear_test$.resid)^2)/nrow(test)) # 5.775885
...
sqrt(sum((fit_linear_test_2$.resid)^2)/nrow(test)) # 5.775885
