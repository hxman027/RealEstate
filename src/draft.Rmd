...
tax_156 <- pct_final %>% filter(Municipalities == municipality.list[i]) %>% 
  filter(TaxClassCode %in% tax.list) %>% 
  ggplot(aes(x=Year, y=tax)) +
   geom_line() + theme(legend.position = "none") + ggtitle("tax amount")

multiplot(pct_rate1,pct_assess1,tax_156)
multiplot(pct_rate5,pct_assess5,tax_156)
multiplot(pct_rate6,pct_assess6,tax_156)
```
